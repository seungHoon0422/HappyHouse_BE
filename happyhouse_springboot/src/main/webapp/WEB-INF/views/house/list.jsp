...
var selectAptCode = '';
// 상세정보 모달 렌더링 시에 선택한 매물에 해당하는 아파트 주소 저장
var selectAddress = '';
var selectApartPos = [];
var selectedStarbucks = [];
var dis = 1.0;
var location ;
var index = 0;
var markers = [];
...
// 		$('#starbucksmap').css("display", "none");
	...
	function drawModal(deal, house) {
		...
		//===================== 모달 지도에 아파트 마커 표시 ========================
		// 세부정보 버튼 클릭한 아파트 주소
		selectAddress = $('#maddress').text();
		// 세부정보 버튼 클릭한 아파트 코드
		selectAptCode = deal.aptCode;

		var container = document.getElementById('starbucksmap'); //지도를 담을 영역의 DOM 레퍼런스
...
$(document).on('click', '#starbucksBtn', function(){
	...
	//=========================== 아파트 마커 표시 ==================================
	console.log('select apart address : ' + selectAddress);
	// 마커의 위치를 저장할 변수
	let apartPos;
	...
	geocoder.addressSearch(selectAddress, function(result, status) {
	    ...
	    // 정상적으로 검색이 완료됐으면 
	     if (status === kakao.maps.services.Status.OK) {
	        ...
	        selectApartPos[0] = result[0].y;
	        selectApartPos[1] = result[0].x;
	...
	console.log('apart position : ' + selectApartPos);	
	//=====================================================================
	var markers = [];
	var container = document.getElementById('starbucksmap'); //지도를 담을 영역의 DOM 레퍼런스
	var options = { //지도를 생성할 때 필요한 기본 옵션
	...
	$.ajax({
		...
		success:function(response, status) {
				...
				geocoder.addressSearch(starbucks['address'], function(result, status) {
				    ...
				    // 정상적으로 검색이 완료됐으면 
				     if (status === kakao.maps.services.Status.OK) {
				        ...
				        // 스타벅스 매장과 선태한 아파트 사이의 거리
				        var distance = getDistanceFromLatLonInKm(selectApartPos[0], selectApartPos[1], result[0].y, result[0].x);
						
						// 거리가 더 가까운 매장이 있으면 지도의 중심 세팅
						if(distance < dis){
							dis = distance;
							selectedStarbucks[0] = result[0].y;
							selectedStarbucks[1] = result[0].x;
						    map.setCenter(coords);
						}
				        ...
				        var imageSrc = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqYD6XNFKFgm1acezUXAwm6fVO6osxMtWZVg&usqp=CAU';    
			...
			} // let starbucks foreach response
			...
			<div class="row mt-3">
				<table class="table table-striped">
					...
					<c:forEach var="info" items="${infos }">
						<tr>
							<input type="hidden" id="aptCode${info.no}" name="aptCode" value="${info.aptCode}"/>
